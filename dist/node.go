...
func NewNode(id, weight int) *Node {
	return &Node{
		...
		inbox:      make(chan Message_Node, 10), // Используем буферизованный канал
	}
}
...
func (n *Node) StartElection(nodes []Node) {
	...
	for _, node := range nodes {
		if node.id != n.id && node.alive {
			msg := Message_Node{kind: "ELECTION", fromID: n.id, weight: n.weight}
			node.inbox <- msg
		}
	}
	...
	go func() {
		...
		if n.leaderID == -1 { // Если лидер не установлен
			...
			for _, node := range nodes {
				if node.id != n.id && node.alive {
					node.inbox <- Message_Node{kind: "COORDINATOR", fromID: n.leaderID}
				}
			}
		}
		n.mu.Unlock()
...
func (n *Node) ProcessMessage(msg Message_Node) {
	n.mu.Lock()
	defer n.mu.Unlock()
	...
	case "ELECTION":
		...
		if msg.weight < n.weight || (msg.weight == n.weight && msg.fromID < n.id) {
			...
			n.inbox <- Message_Node{kind: "OK", fromID: n.id, weight: n.weight}
		} else {
	...
	case "COLLECT":
		if n.alive {
			...
			n.inbox <- Message_Node{kind: "COLLECT_REPLY", fromID: n.id, data: n.localCount}
		} else {
...
func (n *Node) StartGlobalCollection(nodes []Node) {
	if n.leaderID == n.id {
		...
		for _, node := range nodes {
			if node.id != n.id && node.alive {
				numNodes++
				node.inbox <- Message_Node{kind: "COLLECT", fromID: n.id}
			}
		}
